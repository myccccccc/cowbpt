...
class LogTest : public testing::Test {
 public:
  ...
  void PrintSourceHex() {
    ...
    using std::string; using std::hex;
    cout << hex;
    for (char ch : source_->contents_.string())
    {
        cout << "0x" << to_uint(ch) << ' '; 
    }
    cout << endl;
  ...
  void CheckInitialOffsetRecord(uint64_t initial_offset,
    ...
    for (; expected_record_offset < num_initial_offset_records_;
      ...
      ASSERT_EQ((char)('a' + expected_record_offset), record.c_string()[0]);
    }
    delete offset_reader;
  }
 ...
 private:
  class StringDest : public WritableFile {
   public:
    ...
    Status Append(const Slice& slice) override {
      contents_.append(slice.c_string(), slice.size());
      return Status::OK();
    }
  ...
  class StringSource : public SequentialFile {
   public:
    ...
    Status Read(size_t n, Slice& result) override {
      ...
      result = Slice(contents_.c_string(), n);
  ...
  std::shared_ptr<StringDest> dest_;
  std::shared_ptr<StringSource> source_;
