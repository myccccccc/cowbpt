...
TEST_F(EnvTest, ReadWrite) {
  ...
  std::string test_dir;
  ASSERT_COWBPT_OK(env_->GetTestDirectory(&test_dir));
  std::string test_file_name = test_dir + "/open_on_read.txt";
  WritableFilePtr writable_file;
  ASSERT_COWBPT_OK(env_->NewWritableFile(test_file_name, writable_file));
  ...
  while (data.size() < kDataSize) {
    ...
    ASSERT_COWBPT_OK(writable_file->Append(r));
    data += r;
    if (rnd.OneIn(10)) {
      ASSERT_COWBPT_OK(writable_file->Flush());
    }
  }
  ASSERT_COWBPT_OK(writable_file->Sync());
  ASSERT_COWBPT_OK(writable_file->Close());

  // Read all data using a sequence of randomly sized reads.
  SequentialFilePtr sequential_file;
  ASSERT_COWBPT_OK(env_->NewSequentialFile(test_file_name, sequential_file));
  ...
  while (read_result.size() < data.size()) {
    ...
    Slice read;
    ASSERT_COWBPT_OK(sequential_file->Read(len, read));
    if (len > 0) {
      ASSERT_GT(read.size(), 0);
    }
    ASSERT_LE(read.size(), len);
    read_result.append(read.c_string(), read.size());
  }
...
TEST_F(EnvTest, TestOpenNonExistentFile) {
  ...
  RandomAccessFilePtr random_access_file;
  Status status =
...
TEST_F(EnvTest, ReopenWritableFile) {
  ...
  ASSERT_COWBPT_OK(writable_file->Append(data));
  ASSERT_COWBPT_OK(writable_file->Close());
  ...
  ASSERT_COWBPT_OK(ReadFileToString(env_, test_file_name, &data));
...
TEST_F(EnvTest, ReopenAppendableFile) {
  ...
  WritableFilePtr appendable_file;
  ASSERT_COWBPT_OK(env_->NewAppendableFile(test_file_name, appendable_file));
  std::string data("hello world!");
  ASSERT_COWBPT_OK(appendable_file->Append(data));
  ASSERT_COWBPT_OK(appendable_file->Close());
